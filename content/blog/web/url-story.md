...
사람들은 웹 브라우저를 띄워 편하게 그들이 기억하고 있는 주소를 입력하여 웹 페이지에 접근한다. 친구들이나 팀원들끼리도 특정 웹 사이트의 내용을 공유할 때 링크 또는 URL을 주고 받는다는 표현을 많이 쓴다. 이렇게 URL이란 단어는 위화감 없이 일상에서 쓰이고 있다. 이 포스트는 URL과 그 친구들에 대해서 알아보도록 한다.
...
예를 들어서 당장 이 포스트의 주소만 하더라도 `https://gracieuxyh.dev/web/url-story`이다. 이는 gracieuxyh.dev라는 도메인 네임을 가진 서버의 web 디렉토리 밑에 url-story란 파일이 있는 것을 말할까? 당연하게도 아니다. 어느 특정 경로에 위치한 이 포스트 파일(리소스)의 주소를 시맨틱하게 rewrite하여 제공함으로서 보다 직관적이고 알기 쉬운 웹 환경을 조성한다.
...
웹의 주소, 호스트의 주소라고 불리는 것의 실체는 **IP(Internet Protocol) 주소**이다. 컴퓨터 간의 네트워크 통신을 위해서는 상대 컴퓨터가 네트워크 상에서 어디에 위치하는지(주소)를 알아야한다. IP 주소를 통해서 상대 컴퓨터의 위치를 파악할 수 있는 것이다. IP 주소는 IPv4 기준 32bit, 총 12개의 수로 이루어져있고 0.0.0.0부터 시작하여
255.255.255.255까지 나타낼 수 있다.
...
근데 컴퓨터는 도메인 네임을 이해할 수 있을까? 근본적으로 말하자면, 아니다. 도메인 네임은 웹에 존재하는 리소스나 호스트의 주소를 인간이 이해하기 쉽게 인간의 언어로 표현한 것이다. 이렇게 리소스나 호스트의 주소를 도메인 네임으로 바꿔주는 것을 **도메인 네임 시스템(Domain Name System, DNS)**라고 한다. 물론 그 반대로 도메인 네임을 컴퓨터가 이해할 수 있는 IP 주소로로 바꾸기도 한다.
